 ...
 *                already there).
 *
 *   g_wv       : the webview::webview instance. Created by Ascaridol.run on
 *                main, lives until run() returns. Workers reading via
 *                acquire-load see either the valid pointer or null;
 *                both are safe.
 *
 * NOT in include/. Internal to mruby-webview; both src/ and tools/ascaridol/
 * include this header. Declarations live here, definitions in
 * tools/ascaridol/ascaridol.cpp.
 */
...
/* ascaridol_in_main_state — true iff the caller's mrb_state is the one main()
...
static inline bool
ascaridol_in_main_state(mrb_state* caller_mrb)
{
    return caller_mrb == g_main_mrb.load(std::memory_order_acquire);
}

/* ========================================================================= */
/* Error helpers — translate webview_error_t -> Ascaridol::<XxxError>.           */
/*                                                                           */
/* ascaridol_error_class    : resolve subclass; falls back to Ascaridol::Error.      */
/* ascaridol_check          : raise unless code == WEBVIEW_ERROR_OK.             */
/* ascaridol_check_result   : for webview's `result<T>`; raises on !ok().        */
/*                                                                           */
...
struct RClass* ascaridol_error_class(mrb_state* mrb, webview_error_t err);
void           ascaridol_check(mrb_state* mrb, webview_error_t code, const std::string& msg);

template <typename R>
void
ascaridol_check_result(mrb_state* mrb, const R& r)
{
    if (r.ok()) return;
    const auto& info = r.error();
    ascaridol_check(mrb, info.code(), info.message());
}
...
/* raise from ascaridol_check_result longjmps into the local frame instead of  */
/* unwinding through webview::run(). On error, prints + clears mrb->exc.    */
template <typename F>
inline void
ascaridol_protect_on_main(F fn)
{
