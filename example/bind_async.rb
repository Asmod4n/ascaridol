# Async bind example for Ascaridol — Promise resolution from Ruby.
#
# Build:
#   conf.gem '<path-to-ascaridol-mrb>' do |ascaridol|
#     ascaridol.ascaridol_main = File.expand_path('example/bind_async.rb', __dir__)
#   end
#   rake
#
# Run:
...
# later with Ascaridol.resolve. Demonstrates FIFO queueing of waiters.
def main
  Ascaridol.run(title: "resolve demo", size: [400, 200]) do |h|
    @pending = []
    @counter = 0
